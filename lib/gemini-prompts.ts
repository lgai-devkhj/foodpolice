...
function getCorePrinciplesBlock(mode: PromptMode = 'standard'): string {
  if (mode === 'fast') {
    return joinLines(
      ...
      '- 가능하면 ingredientSignals를 채워서 성분별 역할과 제품 구조를 함께 설명해요.',
...
function getFoodPoliceFinalCriteriaBlock(mode: PromptMode = 'standard'): string {
  if (mode === 'fast') {
    return joinLines(
      ...
      '- 원재료 개수가 많거나 종류가 다양한 것 자체는 Group 4 근거가 아니에요.',
      '- 단순 혼합(mixed nuts, salad, grain mix)은 초가공 판단 근거가 아니에요.',
      '- 여러 자연 식재료가 섞여 있어도 향료·감미료·유화제·분해 성분 같은 초가공 신호가 없으면 초가공으로 올리지 않아요.',
      '- 4A: 분해 성분 개수 ≤ 1 이고 첨가물 개수 ≤ 2',
      '- 4B: 4A가 아니면서 S ≤ 7',
      '- 4C: 나머지 Group 4',
      '',
      '[ingredientSignals 출력]',
      '- 가능하면 ingredientSignals를 채우고, 분류 계산값과 일치시켜요.',
      '- structureType: simple_single | simple_mix | recomposed | coated_or_flavored_mix',
      '- dominanceType: whole_base_dominant | mixed_balanced | ultra_signal_dominant',
    );
  }
  ...
  return joinLines(
    ...
    '3) Group 3',
    '4) 나머지는 Group 4',
    '5) Group 4 세분화: 4A → 4B → 4C',
    '',
    '[NOVA 1~4 분류]',
    '- Group 2: 원재료 1개 + 조리용 재료',
    '- Group 1: 원재료 1개 + 분해 0 + 첨가물 0 + 조리용 아님',
    '- Group 3: 원재료 2개 이상 + 분해 0 + 첨가물 2개 이하 + 핵심 첨가물 없음',
    '- Group 4: 분해 1개 이상 또는 첨가물 3개 이상 또는 핵심 첨가물 포함',
    '- 재료 다양성/단순 혼합(mixed nuts, salad, grain mix)은 단독으로 Group 4 근거가 아니에요.',
    '',
    '[Group 4 세분화]',
    '- 4A: 분해 1개 이하 + 첨가물 2개 이하',
    '- 4B: 4A가 아니면서 S ≤ 7',
    '- 4C: 나머지 Group 4',
    '',
    '[중요]',
    '- 분류는 분해 성분 개수·첨가물 개수·S를 우선해요.',
    '- 기준 충돌 시 위 규칙을 우선해요.',
    '',
    '[ingredientSignals 출력]',
    '- 가능하면 ingredientSignals를 채우고, 분류 계산값과 일치시켜요.',

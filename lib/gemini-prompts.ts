...
function getFoodPoliceFinalCriteriaBlock(mode: PromptMode = 'standard'): string {
  if (mode === 'fast') {
    return joinLines(
      ...
      '- 4B: 4C가 아니면서 (분해 성분 2개 이상) 또는 (분해 성분 1개 이상 + 첨가물 2개 이상)',
      '- 4A: 나머지 Group 4 (핵심 첨가물만 있거나, 분해 성분 1개 + 첨가물 1개 수준은 4A)'
    );
  }
  ...
  return joinLines(
    ...
    '- 4B: 4C가 아니면서 (분해 성분 2개 이상) 또는 (분해 성분 1개 이상 + 첨가물 2개 이상)이면 4B예요.',
    '- 4A: 나머지 Group 4는 4A예요. 핵심 첨가물만 있는 경우, 또는 분해 성분 1개 + 첨가물 1개 수준은 4A예요.',
    '',
    '[중요]',
